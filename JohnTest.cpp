...
#include "stol_display.hpp"
#include "radarbox.hpp"
#include "vehiclenetwork.hpp"
#include "dgps_box.hpp"
//#include "dgps_base.hpp"
#include "sharedmem.hpp"
#include "stol_logger.hpp"
#include "cellphone.hpp"
#include "udpsocket_port.hpp"
#include "savari_bsm_data.hpp"
//#include "savari_logger.hpp"

using namespace boost::interprocess;
using namespace std;

VehicleData *pVNData;
CellPhoneData *pcellphonedata;
RadarData *FrontRadarShared;
DGPSdata *pDGPSData;
...
static int GetACC(REALobject instance);
static double GetDistance(REALobject instance);
static double GetHeading(REALobject instance);
static double GetLat(REALobject instance);
static double GetLong(REALobject instance);
static double GetSetSpeed(REALobject instance);
static double GetSpeed(REALobject instance);
static int GetTimeGapSetting(REALobject instance);
static double GetVSLLastUpdate(REALobject instance);
static int GetVSLMode(REALobject instance);
static double GetVSLSpeed(REALobject instance);
...
//	{ "", "last_i2v_rx", "Double", REALconsoleSafe, REALstandardGetter, REALstandardSetter, FieldOffset( TestClassData, mlast_i2v_rx) },
	{ "", "last_i2v_rx", "Double", REALconsoleSafe, (REALproc)GetVSLLastUpdate, (REALproc)GetVSLLastUpdate},
};
...
REALmethodDefinition TestClassMethods[] = {
	...
	{ (REALproc)MyGetString, REALnoImplementation, "GetString() as String", REALconsoleSafe },		// Part of the StringProvider interface
        ...
        { (REALproc)GetACC, REALnoImplementation, "GetACC() as Int32", REALconsoleSafe },
        { (REALproc)GetDistance, REALnoImplementation, "GetDistance() as Double", REALconsoleSafe },
        { (REALproc)GetHeading, REALnoImplementation, "GetHeading() as Double", REALconsoleSafe },
        { (REALproc)GetLat, REALnoImplementation, "GetLat() as Double", REALconsoleSafe },
        { (REALproc)GetLong, REALnoImplementation, "GetLong() as Double", REALconsoleSafe },
        { (REALproc)GetSetSpeed, REALnoImplementation, "GetSetSpeed() as Double", REALconsoleSafe },
        { (REALproc)GetSpeed, REALnoImplementation, "GetSpeed() as Double", REALconsoleSafe },
        { (REALproc)GetTimeGapSetting, REALnoImplementation, "GetTimeGapSetting() as Int32", REALconsoleSafe },
        { (REALproc)GetVSLLastUpdate, REALnoImplementation, "GetVSLLastUpdate() as Double", REALconsoleSafe },
        { (REALproc)GetVSLMode, REALnoImplementation, "GetVSLMode() as Int32", REALconsoleSafe },
        { (REALproc)GetVSLSpeed, REALnoImplementation, "GetVSLSpeed() as Double", REALconsoleSafe },
...
static void TestClassInitializer( REALobject instance )
{
	...
	me->mlast_i2v_rx = 0;
        ...
        SharedMem<VehicleData> shmVN(VEHICLE_NETWORK_DATA_P,POSIX_READ_ONLY,0,1);
        pVNData = shmVN.createSharedMemory();

        SharedMem<CellPhoneData> shmCP(CELL_PHONE_DATA_P,POSIX_READ_ONLY,0,1);
        pcellphonedata = shmCP.createSharedMemory();

        SharedMem<RadarData> shmF(F_RADAR_DATA_P, POSIX_READ_ONLY,0,1);
        FrontRadarShared = shmF.createSharedMemory();

        SharedMem<DGPSdata> shmDGPS(DGPS_DATA, POSIX_READ_ONLY, 0, 1);
        pDGPSData = shmDGPS.createSharedMemory();
...
int GetACC( REALobject instance ) {
	...
	ClassData( TestClassDefinition, instance, TestClassData, me );
		...
		accstatus = pVNData->accData.accActiveFlag;
                ...
                cout << "Vehicle Network shared memory is LOCKED" << endl;
        }

	return (int)accstatus;
}
...
double GetHeading( REALobject instance ) {
	...
	double heading =  -1234.567;

        named_mutex mutexDGPS(open_only, DGPS_MUTEX);

	ClassData( TestClassDefinition, instance, TestClassData, me );

	if(mutexDGPS.try_lock())
	{
		printf("heading: %f\n", pDGPSData->heading);
		heading = pDGPSData->heading;

		mutexDGPS.unlock();
	}
	else {
                cout << "DGPS shared memory is LOCKED" << endl;
	}

	return (double)heading;
}

double GetLat( REALobject instance ) {

	double latitude =  -1234.567;
...
double GetLong( REALobject instance ) {

	double longitude =  -1234.567;
...
double GetSetSpeed( REALobject instance ) {
	...
	ClassData( TestClassDefinition, instance, TestClassData, me );
        ...
        if(VNnm.try_lock()){
		setspeed = pVNData->accData.accSpeedSetpoint;
                VNnm.unlock();
        }
        else    {
                cout << "Vehicle Network shared memory is LOCKED" << endl;
...
double GetSpeed( REALobject instance ) {
	...
	ClassData( TestClassDefinition, instance, TestClassData, me );
        ...
        if(VNnm.try_lock()){
                cout << "WheelSpeed=" << pVNData->wheelSpeed[0] << " VehicleSpeed=" << pVNData->vehicleSpeed << endl;
		wheelspeed = pVNData->wheelSpeed[0];
		vehiclespeed = pVNData->vehicleSpeed; 
                VNnm.unlock();
        }
        else    {
                cout << "Vehicle Network shared memory is LOCKED" << endl;
	...
	return (double)vehiclespeed;
}

int GetTimeGapSetting( REALobject instance ) {

	int timegapsetting = -1234;
	...
	ClassData( TestClassDefinition, instance, TestClassData, me );
        ...
        if(VNnm.try_lock()){
                cout << "Time gap setting " << pVNData->accData.accHeadwaySetting << endl;
		timegapsetting = pVNData->accData.accHeadwaySetting;
                VNnm.unlock();
        }
        else    {
                cout << "Vehicle Network shared memory is LOCKED" << endl;
	...
	return (int)timegapsetting;
}


double GetVSLLastUpdate( REALobject instance ) {

	int last_i2v_rx = -1234;

	named_mutex CPnm(open_only,CELL_PHONE_MUTEX);

	ClassData( TestClassDefinition, instance, TestClassData, me );

        if(CPnm.try_lock()){
                last_i2v_rx = (int) (pcellphonedata->last_i2v_rx);
		REALLockObject(instance);
		me->mlast_i2v_rx = last_i2v_rx;
		REALUnlockObject(instance);
                cout << "Last_I2V_Rx: " << last_i2v_rx << endl;
                CPnm.unlock();
        }
	else {
                cout << "Cell phone shared memory is LOCKED" << endl;
	}

	return (double)last_i2v_rx;
}

int GetVSLMode( REALobject instance ) {
	...
	ClassData( TestClassDefinition, instance, TestClassData, me );
        ...
        if(CPnm.try_lock()){
		vslmode = pcellphonedata->ACCstatus;
                cout << "VSL Mode: " << vslmode << endl;
                CPnm.unlock();
        }
	else {
                cout << "Cell phone shared memory is LOCKED" << endl;
...
double GetVSLSpeed( REALobject instance ) {
	...
	ClassData( TestClassDefinition, instance, TestClassData, me );
        ...
        if(CPnm.try_lock()){
		vslspeed = pcellphonedata->speedMPH;
                cout << "VSL speed: " << vslspeed << endl;
                CPnm.unlock();
        }
	else {
                cout << "Cell phone shared memory is LOCKED" << endl  << endl << endl;
	}

	return (double)vslspeed;
}
